/*
  # Achievement System Enhancements (Schema Compatible)
  ...
  2. Enhanced Tables
    - `achievements` - Add category, difficulty, and reward information
  ...
  3. Security
    ...
    - Add policies for reading achievement definitions

  4. Performance
    - Add indexes for common queries
    - Add triggers for automatic progress calculation

  Note: Updated to work with existing integer-based achievement IDs
*/
...
WHERE NOT EXISTS (SELECT 1 FROM achievements WHERE title = 'First Workout');
...
WHERE NOT EXISTS (SELECT 1 FROM achievements WHERE title = 'Workout Warrior');
...
SELECT 
  ...
  'hard',
  (SELECT id FROM achievement_categories WHERE name = 'Consistency')
WHERE NOT EXISTS (SELECT 1 FROM achievements WHERE title = 'Streak Master');
...
WHERE NOT EXISTS (SELECT 1 FROM achievements WHERE title = 'Volume Beast');
...
WHERE NOT EXISTS (SELECT 1 FROM achievements WHERE title = 'Personal Best');

-- Update existing achievements with categories (if they exist and don't have categories)
UPDATE achievements SET category_id = (
  SELECT id FROM achievement_categories WHERE name = 'Consistency'
) WHERE category_id IS NULL AND (title ILIKE '%streak%' OR title ILIKE '%consistency%' OR title ILIKE '%habit%');

UPDATE achievements SET category_id = (
  SELECT id FROM achievement_categories WHERE name = 'Strength'
) WHERE category_id IS NULL AND (title ILIKE '%strength%' OR title ILIKE '%record%' OR title ILIKE '%pr%');

UPDATE achievements SET category_id = (
  SELECT id FROM achievement_categories WHERE name = 'Volume'
) WHERE category_id IS NULL AND (title ILIKE '%volume%' OR title ILIKE '%weight%' OR title ILIKE '%total%');

UPDATE achievements SET category_id = (
  SELECT id FROM achievement_categories WHERE name = 'Milestones'
) WHERE category_id IS NULL AND (title ILIKE '%first%' OR title ILIKE '%milestone%');
