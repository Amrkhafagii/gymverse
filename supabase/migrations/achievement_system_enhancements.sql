/*
  ...
  4. Performance
    ...
    - Add materialized view for achievement leaderboards
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'achievements' AND column_name = 'category_id') THEN
    ALTER TABLE achievements ADD COLUMN category_id uuid REFERENCES achievement_categories(id);
  END IF;
...
CREATE OR REPLACE FUNCTION calculate_achievement_progress(p_user_id uuid, p_achievement_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  VALUES (p_user_id, p_achievement_id, progress_value, achievement_record.target_value)
...
BEGIN
  ...
  SELECT COUNT(*) INTO total_count FROM achievements WHERE is_active = true;
  ...
  SELECT COALESCE(SUM(a.reward_points), 0) INTO total_points_earned
...
) WHERE type IN ('personal_records', 'max_weight');
