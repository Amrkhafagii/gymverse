/*
  # Achievement System Final Migration (Criteria JSON Format Fixed)
  ...
  Note: Includes name, title, and criteria columns with proper JSON formatting
*/
...
-- Wait for categories to be inserted, then add achievements with name, title, and criteria in JSON format
DO $$
BEGIN
  -- Only insert achievements if none exist with these names
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'first_workout') THEN
    INSERT INTO achievements (name, title, description, icon, type, target_value, reward_points, difficulty, category_id, criteria) 
    VALUES (
      'first_workout',
      'First Workout',
      ...
      '{"description": "Complete 1 workout"}'::json
    );
  END IF;

  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'workout_warrior') THEN
    INSERT INTO achievements (name, title, description, icon, type, target_value, reward_points, difficulty, category_id, criteria) 
    VALUES (
      'workout_warrior',
      'Workout Warrior',
      ...
      '{"description": "Complete 10 workouts"}'::json
    );
  END IF;

  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'streak_master') THEN
    INSERT INTO achievements (name, title, description, icon, type, target_value, reward_points, difficulty, category_id, criteria) 
    VALUES (
      'streak_master',
      'Streak Master',
      ...
      'hard',
      (SELECT id FROM achievement_categories WHERE name = 'Consistency'),
      '{"description": "Maintain 7-day streak"}'::json
    );
  END IF;

  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'volume_beast') THEN
    INSERT INTO achievements (name, title, description, icon, type, target_value, reward_points, difficulty, category_id, criteria) 
    VALUES (
      'volume_beast',
      'Volume Beast',
      ...
      '{"description": "Lift 10,000kg total"}'::json
    );
  END IF;

  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'personal_best') THEN
    INSERT INTO achievements (name, title, description, icon, type, target_value, reward_points, difficulty, category_id, criteria) 
    VALUES (
      'personal_best',
      'Personal Best',
      ...
      '{"description": "Set 1 personal record"}'::json
    );
  END IF;
END $$;
...
-- Update existing achievements with default criteria in JSON format if null
UPDATE achievements SET criteria = '{"description": "Achievement criteria"}'::json WHERE criteria IS NULL;
