/*
  # Achievement System Final Migration (Name Column Fixed)
  ...
  Note: Includes both name and title columns for achievements table
*/
...
-- Wait for categories to be inserted, then add achievements with BOTH name and title
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'first_workout') THEN
    INSERT INTO achievements (name, title, description, icon, type, target_value, reward_points, difficulty, category_id) 
    VALUES (
      'first_workout',
      'First Workout',
