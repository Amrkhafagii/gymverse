...
CREATE TABLE IF NOT EXISTS photo_analytics (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES auth.users(id) NOT NULL,
  photo_id uuid REFERENCES progress_photos(id) NOT NULL,
...
CREATE TABLE IF NOT EXISTS photo_shares (
  ...
  created_at timestamptz DEFAULT now(),
  CONSTRAINT photo_or_comparison_check CHECK (
...
CREATE INDEX IF NOT EXISTS idx_progress_photos_album_id ON progress_photos(album_id);
CREATE INDEX IF NOT EXISTS idx_progress_photos_date ON progress_photos(photo_date);
CREATE INDEX IF NOT EXISTS idx_progress_photos_tags ON progress_photos USING GIN(tags);
CREATE INDEX IF NOT EXISTS idx_photo_comparisons_user_id ON photo_comparisons(user_id);
CREATE INDEX IF NOT EXISTS idx_photo_tags_user_id ON photo_tags(user_id);
CREATE INDEX IF NOT EXISTS idx_photo_analytics_user_id ON photo_analytics(user_id);
...
-- Create default album for each user
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE trigger_name = 'create_default_album_trigger'
  ) THEN
