...
import { Play, Pause, Square, Plus, Minus, Clock, Target, CircleCheck as CheckCircle, X, Timer } from 'lucide-react-native';
...
export default function WorkoutSessionScreen() {
  ...
  const logSet = async (setIndex: number, reps: number, weight: number, duration?: number) => {
  ...
  const finishWorkout = async () => {
    ...
    try {
      ...
      const estimatedCalories = Math.floor(duration * 5); // Simple estimation
      ...
      let message = `Great job! You completed your workout in ${duration} minutes.`;
...
interface SetLoggerProps {
  ...
  onLogSet: (index: number, reps: number, weight: number, duration?: number) => void;
}
...
function SetLogger({ set, setIndex, exercise, onLogSet }: SetLoggerProps) {
  ...
  const handleLogSet = () => {
    ...
    onLogSet(setIndex, repsNum, weightNum, durationNum);
  };
